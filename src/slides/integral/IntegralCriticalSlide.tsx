...
interface IntegralCriticalSlideProps {
  onNavigate?: (slideIndex: number) => void;
}

export function IntegralCriticalSlide({ onNavigate }: IntegralCriticalSlideProps) {
  return (
    <div className="slide relative overflow-hidden bg-white">
      ...
      <div className="flex-1 flex flex-col justify-center z-10 max-w-5xl mx-auto w-full px-8">
        ...
        >
          <span className="text-orange-500">النقدية</span>
        </motion.h2>
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          >
            <p className="text-xl sm:text-2xl leading-relaxed text-gray-700 text-right" style={{ textAlign: 'justify', direction: 'rtl' }}>
              عمليات التقييم المستمرة ووضع أهداف ومعايير واضحة جزء من عمليات التطوير والتصحيح لكن يتعدى الأمر لتبني النقدية كجزء من المهارات والثقافة المطلوبة للنجاح.
            </p>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            <p className="text-xl sm:text-2xl leading-relaxed text-gray-700" dir="ltr" style={{ textAlign: 'justify' }}>
              Continuous evaluation processes and setting clear goals and standards are part of development and correction operations. But it goes beyond that to embrace critical thinking as part of the skills and culture required for success.
            </p>
          </motion.div>
        </div>
